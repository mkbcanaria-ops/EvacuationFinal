...
class _ResidentRegistrationPageWidgetState
  ...
  Future<void> _registerSave() async {
    ...
    final uid = currentUser.id; // Use this UID everywhere
    ...
    try {
      // ✅ Prevent duplicate disaster record for this user
      final existing = await supabase
          .from('Registration_Table')
          .select('Registration_ID')
          .eq('Type_of_Disaster', disasterType)
          .eq('Date_of_Occurrence', date)
          .eq('UID', uid) // Check for current user's record
          .maybeSingle();
      ...
      if (existing != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('You already submitted this disaster record.'),
