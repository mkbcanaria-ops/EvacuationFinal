...
class AppSantaNationalHighSchoolResidentsPage extends StatefulWidget {
  const AppSantaNationalHighSchoolResidentsPage({super.key});

  @override
  State<AppSantaNationalHighSchoolResidentsPage> createState() =>
      _AppSantaNationalHighSchoolResidentsPageState();
}

class _AppSantaNationalHighSchoolResidentsPageState
    extends State<AppSantaNationalHighSchoolResidentsPage> {
  ...
  Future<void> _fetchResidents() async {
    ...
    try {
      ...
      // Only fetch from Evacuation_A — discharges only supported from Evacuation_A
      final combined = evacA;
  ...
  Future<void> _dischargeResidents(String registrationId, String uid) async {
    ...
    // Step 1: Check if resident is currently in Evacuation_A only
    ...
    if (evacAData == null) {
      // Resident is not in Evacuation_A — nothing to discharge
      if (mounted) {
    ...
    // Step 3: Delete existing entries from Evacuation_A only
    try {
      await supabase.from('Evacuation_A').delete().eq('UID', uid);
      debugPrint('🧹 Removed resident from Evacuation_A for UID: $uid');
    } catch (e) {
      debugPrint('⚠️ Error deleting from Evacuation_A: $e');
    }
